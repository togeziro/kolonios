...
| Function | Method | Required permission | Payload | Returns |
| -------- | ------ | ------------------- | ------- | ------- |
| `listSalaryComponentsFn` | GET | `payroll.view` | `PaginationQuery` | `SalaryComponent[]` |
| `createSalaryComponentFn` | POST | `payroll.add` | `SalaryComponentPayload` | `SalaryComponent` (+ audit) |
| `updateSalaryComponentFn` | POST | `payroll.edit` | `{ id, values }` | `SalaryComponent` (+ audit) |
| `deleteSalaryComponentFn` | POST | `payroll.delete` | `number` (id) | `{ success, message }` (+ audit) |
| `getEmployeePayrollProfileFn` | GET | `payroll.view` | `employeeIdSchema` (id) | `EmployeePayrollProfile` |
| `updateEmployeeSalaryAssignmentFn` | POST | `payroll.edit` | `{ employeeId, values }` | `SalaryAssignment` (+ audit) |
| `updateEmployeeTaxProfileFn` | POST | `payroll.edit` | `{ employeeId, values }` | `TaxProfile` (+ audit) |
| `updateEmployeeBenefitEnrollmentFn` | POST | `payroll.edit` | `{ employeeId, values }` | `BenefitEnrollment` (+ audit) |
| `updateEmployeeBankAccountFn` | POST | `payroll.edit` | `{ employeeId, values }` | `BankAccount` (+ audit) |
| `updateEmployeeEmploymentEventFn` | POST | `payroll.edit` | `{ employeeId, values }` | `EmploymentEvent` (+ audit) |
| `listPayrollPeriodsFn` | GET | `payroll.view` | `PaginationQuery` | `PayrollPeriod[]` |
| `createPayrollPeriodFn` | POST | `payroll.add` | `PayrollPeriodPayload` | `PayrollPeriod` (+ audit) |
| `updatePayrollPeriodStatusFn` | POST | `payroll.edit` | `{ id, status }` | `PayrollPeriod` (+ audit) |
| `generatePayrollFn` | POST | `payroll.add` | `{ payrollPeriodId }` | `{ records: PayrollRecord[], report: PayrollReport }` (+ audit) |
| `getPayrollRecordsFn` | GET | `payroll.view` | `PayrollRecordFilters` | `PayrollRecord[]` |
| `getPayrollRecordByIdFn` | GET | `payroll.view` | `payrollRecordIdSchema` (id) | `PayrollRecord` |
| `approvePayrollFn` | POST | `payroll.approve` | `{ payrollRecordId }` | `PayrollRecord` (+ audit) |
| `markPayrollPaidFn` | POST | `payroll.pay` | `{ payrollRecordId }` | `PayrollRecord` (+ audit) |
| `lockPayrollFn` | POST | `payroll.edit` | `{ payrollPeriodId }` | `PayrollPeriod` (+ audit) |
| `adjustPayrollRecordFn` | POST | `payroll.edit` | `{ payrollRecordId, input }` | `PayrollRecord` (+ audit) |
| `listMyPayslipsFn` | GET | `payroll.view` | `PaginationQuery` | `Payslip[]` |
| `getMyPayslipFn` | GET | `payroll.view` | `payslipIdSchema` (id) | `Payslip` |
| `downloadMyPayslipFn` | GET | `payroll.view` | `{ payslipId }` | `Blob` (PDF) |
| `exportPayrollReportFn` | POST | `payroll.reports` | `PayrollReportFilters` | `Blob` (CSV/XLSX) |

> All payroll server functions enforce `requireSession()` and `requirePermission(module, action)` at the handler boundary. Sensitive fields (`tax_identifier`, `account_number`) are masked in employee-scoped responses. Payslip PDF responses redact bank account numbers and use company data from the `payroll_records.calculation_snapshot`.
