...
- **Payroll module** — Full payroll calculation engine (monthly/daily/hourly salary types, fixed/percentage/per-attendance/manual components, configurable absence/late/unpaid-leave deductions, progressive + TER tax), payslip PDF generation via `pdf-lib`, admin UI with TanStack Table, employee self-service; MVP excludes overtime calculation; effective-dated salary/tax/benefit/bank/employment data with immutable calculation snapshots
...
- [x] Payroll schema and migration (13 tables, enums, indexes, relations)
- [x] Effective-dated data access layer with salary/tax/benefit/bank/employment resolution
- [x] Pure calculation engine (monthly/daily/hourly, fixed/percentage/per-attendance/manual components, progressive + TER tax, zero overtime seam)
- [x] Validated server functions and client hooks with permission keys and React Query
- [x] Admin payroll UI (overview, components, profiles, periods, generate/review, records with TanStack Table, reports with CSV/XLSX export)
- [x] Employee payslip PDF generation via `pdf-lib` and self-service route
- [x] Seed data, audit events, and E2E verification
- [x] Documentation (design spec, implementation plan, work plan)
