...
host (`apt-get install -y --no-install-recommends nodejs`). `bun install`
runs the repo's `prepare` lifecycle script (`simple-git-hooks`, a `node`
shebang) — without node the deploy fails at dependency install. Any recent
node works; git hooks themselves are a no-op on prod.
...
first account, use the reusable admin script in bootstrap mode (primary
path — one command, registration stays closed, no restart). Deploy this
release first so migration `0039` (`user.must_change_password`) and the
rotation gate are already live:
...
The script generates a unique random password per machine, prints it ONCE
for the operator to copy, hashes via better-auth (scrypt, same path as the
seed), grants `role='admin'` + `zzzrg-admin`, and sets
`must_change_password=true`, so the first login is confined to
`/dashboard/change-password` until the owner sets a fresh password (see
`src/lib/auth/password-gate.ts`). Hand the one-time password over
out-of-band; never write it to a file. Re-runs for the same email always
succeed (grants verified, password untouched, nothing printed). Full
ceremony incl. hardening and lockout recovery: see `docs/INITIAL_LOGIN.md`.

Fallback (only if the script path is unavailable): temporarily open
registration, register, then close it again (never rely on the seed):
...
```bash
cd /opt/kolonios
# Defaults to the pre-deploy revision recorded by the last deploy.sh run:
APP_DIR=/opt/kolonios bash deploy/rollback.sh [<previous-good-sha>]
```

`deploy.sh` records every run in `/var/backups/kolonios/` (`pre-deploy-sha`
before, `last-good-sha` after a green health check), so the rollback target is
usually already known. `rollback.sh` checks out the SHA and re-runs the full
deploy pipeline (install → build → migrate `--no-seed` → restart → health).
...
Live secrets exist in exactly one place: `/etc/kolonios/kolonios.env` on the
VM (mode `640`, `root:kolonios`). Never keep a copy with live values inside
the repo working tree — the wizard output (`deploy/kolonios.prod.env`) must be
moved out after install (e.g. `~/.config/`, mode `600`).

```bash
# On the VM: replace ONE value, then restart.
sudo nano /etc/kolonios/kolonios.env
sudo systemctl restart kolonios
curl -fsS http://127.0.0.1:3000/api/v1/health
```

Per-secret notes (generate replacements with `openssl rand -hex 32`):

| Secret                       | Rotation effect                                                                                                                                                                     |
| ---------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `BETTER_AUTH_SECRET`         | Logs **everyone** out (all sessions invalid). Announce beforehand; rotate off-hours.                                                                                                |
| `STORAGE_ENCRYPTION_KEY`     | Old key must be kept until provider credentials saved through the admin UI are re-saved under the new key — **losing it makes them unrecoverable**. Back the key up off-host first. |
| DB password (`DATABASE_URL`) | Rotate in PostgreSQL (`ALTER ROLE kolonios PASSWORD '…'`) **and** the env file in the same window, then restart.                                                                    |
| `ALLOW_PUBLIC_SIGNUP`        | Not a secret; keep `false` except for the temporary first-admin flow (§3).                                                                                                          |
