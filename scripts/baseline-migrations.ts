...
async function main() {
  const journal = JSON.parse(readFileSync(`${migrationsFolder}/meta/_journal.json`, 'utf-8'));
  ...
  console.log(
    `Baseline complete: ${inserted} migration(s) recorded as applied (${journal.entries.length} total).`
  );
