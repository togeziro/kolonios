...
step "Service user (login-capable so CI can deploy), directories, checkout:"
  say "  id -u kolonios >/dev/null 2>&1 || sudo useradd --system --home-dir /home/kolonios --shell /bin/bash kolonios"
  say "  sudo mkdir -p $DEPLOY_PATH /etc/kolonios /var/backups/kolonios /home/kolonios"
  say "  sudo chown kolonios:kolonios /home/kolonios && sudo chmod 700 /home/kolonios"
...
if [[ -n "$DEPLOY_PUBKEY" ]]; then
  ...
  say "  sudo install -d -m 700 -o kolonios -g kolonios /home/kolonios/.ssh"
  say "  echo '$DEPLOY_PUBKEY' | sudo tee /home/kolonios/.ssh/authorized_keys"
  say "  sudo chown kolonios:kolonios /home/kolonios/.ssh/authorized_keys && sudo chmod 600 /home/kolonios/.ssh/authorized_keys"
else
  note "Authorize your deploy public key in /home/kolonios/.ssh/authorized_keys by hand."
fi
...
if [[ -n "${DEPLOY_SSH_KEY_PATH:-}" && -f "$DEPLOY_SSH_KEY_PATH" ]]; then
  ...
  note "Public key authorized at $DEPLOY_USER@$DEPLOY_HOST:/home/kolonios/.ssh/authorized_keys."
else
