...
export function buildAdminAttendanceColumns({
  onEdit
}: {
  onEdit?: (row: AdminAttendanceReportRow) => void;
}): ColumnDef<AppFeatures, AdminAttendanceReportRow>[] {
  const columns: ColumnDef<AppFeatures, AdminAttendanceReportRow>[] = [
    {
      id: 'date',
    ...
    },
    {
      ...
      cell: ({ row }) => (
        <span className='text-sm tabular-nums'>
          {row.original.attendance.check_out_time ?? '-'}
        </span>
      )
    },
    {
  ...
  if (onEdit) {
    columns.push({
      ...
      cell: ({ row }) => <EditAttendanceCell row={row.original} onEdit={onEdit} />
    });
  }
...
function EditAttendanceCell({
  ...
  return (
    <Button
      variant='ghost'
      size='icon'
      aria-label={t('attendanceAdmin.editAttendanceButton')}
      onClick={() => onEdit(row)}
    >
