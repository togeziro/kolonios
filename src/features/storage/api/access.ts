export type KeyFolder = 'attendance' | 'customers' | 'tickets';

export function parseKeyFolder(key: string): KeyFolder | null {
  const folder = key.split('/')[0];
  return folder === 'attendance' || folder === 'customers' || folder === 'tickets' ? folder : null;
}
 ...
 *   the gate).
 */
export function canViewKey(key: string, userId: string, isAdmin: boolean): boolean {
