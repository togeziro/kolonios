...
// no legs). This map stands in for the future relay lookup DTO — keyed by
...
// JobsPage already accepts the same shape via its `relayMap` prop.
export type TicketRelayInfo = {
  legNumber: number;
  legsTotal: number;
};

export const LEG_FIXTURES: Record<number, TicketRelayInfo> = {
