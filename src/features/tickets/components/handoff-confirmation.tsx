...
export default function HandoffConfirmation({ ticketId }: { ticketId: number }) {
  ...
  const submittedIdx = submitted ? ticket.legs.findIndex((l) => l.id === submitted.id) : -1;
  const nextLeg = submittedIdx >= 0 ? (ticket.legs[submittedIdx + 1] ?? null) : null;
    ...
    <div className='space-y-4 p-4 pb-28'>
      ...
      <Card className='space-y-2 rounded-2xl border p-4 dark:border-zinc-800/50 dark:bg-zinc-900'>
        ...
        {nextLeg ? (
          <div className='flex items-center justify-between'>
            <div>
              ...
              <p className='text-xs text-muted-foreground'>
                {nextLeg.assigneeId ? t('handoff.assignedToUser') : t('handoff.poolFallback')}
              </p>
            </div>
          ...
          <p className='text-sm text-muted-foreground'>{t('handoff.noNextLeg')}</p>
        )}
      </Card>
      ...
      <div className='fixed inset-x-0 bottom-0 border-t bg-background/95 p-3 backdrop-blur dark:border-zinc-800/50 dark:bg-zinc-950/95 max-md:bottom-[calc(5rem+env(safe-area-inset-bottom))]'>
