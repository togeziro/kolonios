...
describe('CareerEventCard — trash visibility', () => {
  it('shows the trash icon when the event is fresh and recorded by the current user', () => {
    ...
    expect(screen.getByTestId('career-event-delete')).toBeInTheDocument();
  });
  ...
  it('hides the trash icon when the event is older than 5 minutes', () => {
    ...
    expect(screen.queryByTestId('career-event-delete')).not.toBeInTheDocument();
  });
...
describe('CareerEventCard — delete confirmation', () => {
  it('opens a confirm dialog showing the from → to description', async () => {
    ...
    await user.click(screen.getByTestId('career-event-delete'));

    expect(await screen.findByText('Delete this event?')).toBeInTheDocument();
    expect(screen.getByText(/Helper → Engineer/)).toBeInTheDocument();
  });
  ...
  it('keeps the event when the dialog is cancelled', async () => {
    ...
    await waitFor(() => expect(screen.queryByText('Delete this event?')).not.toBeInTheDocument());
